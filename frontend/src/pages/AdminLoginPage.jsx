...
const isExternalDestination = (target) =>
    /^https?:\/\//i.test(target) || target.startsWith('/flower');
...
export default function AdminLoginPage() {
    ...
    useEffect(() => {
        ...
        const destination = searchParams.get('next') || location.state?.from || '/admin/celery';

        const go = async () => {
            if (destination.startsWith('/flower')) {
                try {
    ...
    return (
        ...
        >
            ...
            <div className="inline-flex h-12 w-12 rounded-2xl bg-gradient-to-tr from-slate-900 to-purple-800 items-center justify-center shadow-lg shadow-purple-500/20 mb-2">
                <ShieldCheck className="w-6 h-6 text-white" />
            </div>
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Admin Console</h2>
            <p className="text-sm text-slate-500">Sign in to access the operations dashboard</p>
            </div>
            ...
            <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 disabled:opacity-60 disabled:cursor-not-allowed text-white font-medium text-sm flex items-center justify-center space-x-2 shadow-lg shadow-slate-900/20 transition-all duration-200 active:scale-98 mt-2"
            >
