...
LIST_URL = '/api/tasks/'
def detail_url(pk): return f'/api/tasks/{pk}/'
...
class TaskListCreateViewTests(APITestCase):
    ...
    def test_create_task_sets_owner(self):
        self.client.post(LIST_URL, {'title': 'Owned Task', 'status': 'todo', 'priority': 'low'}, format='json')
        task = Task.objects.get(title='Owned Task')
        self.assertEqual(task.owner, self.user)
 
    def test_create_task_without_title_returns_400(self):
        res = self.client.post(LIST_URL, {'status': 'todo', 'priority': 'low'}, format='json')
    ...
    def test_ordering_by_due_date(self):
        make_task(owner=self.user, title='Far Task',  due_date=date.today() + timedelta(days=10))
        make_task(owner=self.user, title='Near Task', due_date=date.today() + timedelta(days=2))
...
class TaskDetailViewTests(APITestCase):
    ...
    def test_patch_updates_task(self):
        res = self.client.patch(detail_url(self.task.id), {'status': 'done'}, format='json')
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_patch_other_user_task_returns_404(self):
        other = make_user(email='other@example.com')
        other_task = make_task(owner=other)
        res = self.client.patch(detail_url(other_task.id), {'status': 'done'}, format='json')
        self.assertEqual(res.status_code, status.HTTP_404_NOT_FOUND)
