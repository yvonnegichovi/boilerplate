...
class LoginViewTests(APITestCase):
    ...
    def test_login_returns_200(self):
        res = self.client.post(self.url, {'email': self.user.email, 'password': DEFAULT_PASSWORD}, format='json')
    ...
    def test_login_wrong_password_returns_401(self):
        res = self.client.post(self.url, {'email': self.user.email, 'password': 'WrongPass!'}, format='json')
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
 
    def test_login_nonexistent_email_returns_401(self):
        res = self.client.post(self.url, {'email': 'ghost@example.com', 'password': DEFAULT_PASSWORD}, format='json')
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
...
class LogoutViewTests(APITestCase):
    ...
    def test_logout_blacklists_refresh_token(self):
        ...
        res = self.client.post('/api/auth/token/refresh/', {'refresh': str(self.refresh)}, format='json')
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
...
class MeViewTests(APITestCase):
    ...
    def test_patch_me_updates_profile(self):
        res = self.client.patch(self.url, {'first_name': 'Janet', 'phone_number': '+254711000000'}, format='json')
        self.assertEqual(res.status_code, status.HTTP_200_OK)
