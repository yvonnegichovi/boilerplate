...
class OrganisationSerializerTests(TestCase):
    def setUp(self):
        ...
        self.org = make_organisation(
            created_by=self.user, name="Rotaract", slug="rotaract"
        )
        make_membership(
    ...
    def test_member_count(self):
        ...
        data = OrganisationSerializer(
            self.org, context={"request": _fake_request(self.user)}
        ).data
    ...
    def test_your_role_none_without_membership(self):
        ...
        data = OrganisationSerializer(
            self.org, context={"request": _fake_request(outsider)}
        ).data
...
class UpdateMemberRoleSerializerTests(TestCase):
    def setUp(self):
        ...
        self.owner_membership = make_membership(
            user=self.owner_user, organisation=self.org, role=Membership.Role.OWNER
        )
...
class AcceptInvitationSerializerTests(TestCase):
    def setUp(self):
        ...
        self.invite = make_invitation(
            organisation=self.org, invited_by=self.org.created_by
        )
