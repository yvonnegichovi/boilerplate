...
def detail_url(slug):
    return f"/api/organisations/{slug}/"


def members_url(slug):
    return f"/api/organisations/{slug}/members/"


def member_detail_url(slug, pk):
    return f"/api/organisations/{slug}/members/{pk}/"


def invitations_url(slug):
    return f"/api/organisations/{slug}/invitations/"


def invitation_detail_url(slug, pk):
    return f"/api/organisations/{slug}/invitations/{pk}/"


def preview_url(token):
    return f"/api/invitations/preview/{token}/"


def accept_url(token):
    return f"/api/invitations/accept/{token}/"
...
class OrganisationListCreateViewTests(APITestCase):
    ...
    def test_create_makes_creator_the_owner(self):
        ...
        membership = Membership.objects.get(
            user=self.user, organisation__slug=res.data["slug"]
        )
...
class OrganisationDetailViewTests(APITestCase):
    def setUp(self):
        self.org = make_organisation(
            created_by=make_user(), name="Detail Org", slug="detail-org"
        )
    ...
    def test_admin_can_update(self):
        ...
        res = self.client.patch(
            detail_url(self.org.slug), {"name": "Renamed"}, format="json"
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_owner_can_update(self):
        ...
        res = self.client.patch(
            detail_url(self.org.slug), {"name": "Renamed Again"}, format="json"
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_member_cannot_update(self):
        auth_client(self.client, self.member)
        res = self.client.patch(
            detail_url(self.org.slug), {"name": "Nope"}, format="json"
        )
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)
...
class MemberViewTests(APITestCase):
    def setUp(self):
        self.org = make_organisation(
            created_by=make_user(), name="Member Org", slug="member-org"
        )
        self.owner = make_user(email="owner@example.com")
        self.admin = make_user(email="admin@example.com")
        self.member = make_user(email="member@example.com")
        ...
        self.admin_membership = make_membership(
            user=self.admin, organisation=self.org, role=Membership.Role.ADMIN
        )
    ...
    def test_admin_can_promote_member(self):
        auth_client(self.client, self.admin)
        res = self.client.patch(
            member_detail_url(self.org.slug, self.member_membership.id),
            {"role": "admin"},
            format="json",
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_member_cannot_change_roles(self):
        auth_client(self.client, self.member)
        res = self.client.patch(
            member_detail_url(self.org.slug, self.admin_membership.id),
            {"role": "member"},
            format="json",
        )
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_cannot_remove_owner(self):
        auth_client(self.client, self.admin)
        res = self.client.delete(
            member_detail_url(self.org.slug, self.owner_membership.id)
        )
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_admin_can_remove_regular_member(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(
            Membership.objects.filter(id=self.member_membership.id).exists()
        )
...
class InvitationViewTests(APITestCase):
    def setUp(self):
        self.org = make_organisation(
            created_by=make_user(), name="Invite Org", slug="invite-org"
        )
        self.owner = make_user(email="owner@example.com")
        self.member = make_user(email="member@example.com")
        make_membership(
    ...
    def test_owner_can_revoke_pending_invitation(self):
        ...
        invite = make_invitation(
            organisation=self.org, invited_by=self.owner, email="pending@example.com"
        )
    ...
    def test_cannot_revoke_already_accepted_invitation(self):
        auth_client(self.client, self.owner)
        invite = make_invitation(
            organisation=self.org, invited_by=self.owner, email="done@example.com"
        )
...
class InvitePreviewAndAcceptViewTests(APITestCase):
    def setUp(self):
        self.org = make_organisation(
            created_by=make_user(), name="Preview Org", slug="preview-org"
        )
        self.owner = make_user(email="owner@example.com")
        make_membership(
            user=self.owner, organisation=self.org, role=Membership.Role.OWNER
        )
        self.invite = make_invitation(
            organisation=self.org, invited_by=self.owner, email="newperson@example.com"
        )
    ...
    def test_accept_authenticated_user_joins_immediately(self):
        ...
        self.assertTrue(
            Membership.objects.filter(
                user=existing_user, organisation=self.org
            ).exists()
        )
