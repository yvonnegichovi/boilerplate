...
class OrganisationModelTests(TestCase):
    ...
    def test_ordering_is_alphabetical_by_name(self):
        make_organisation(created_by=self.owner, name='Aardvark Org', slug='aardvark-org')
...
class MembershipModelTests(TestCase):
    ...
    def test_is_owner_true_only_for_owner_role(self):
        m = make_membership(user=self.user, organisation=self.org, role=Membership.Role.OWNER)
        self.assertTrue(m.is_owner)
        self.assertFalse(m.is_admin)

    def test_is_admin_true_only_for_admin_role(self):
        m = make_membership(user=self.user, organisation=self.org, role=Membership.Role.ADMIN)
    ...
    def test_unique_user_organisation_constraint(self):
        ...
        with self.assertRaises(Exception):
            make_membership(user=self.user, organisation=self.org, role=Membership.Role.MEMBER)
...
class InvitationModelTests(TestCase):
    ...
    def test_default_role_is_member(self):
        invite = make_invitation(organisation=self.org, invited_by=self.owner, email='x@example.com')
        self.assertEqual(invite.role, Membership.Role.MEMBER)

    def test_token_is_generated_and_unique(self):
        i1 = make_invitation(organisation=self.org, invited_by=self.owner, email='a@example.com')
        i2 = make_invitation(organisation=self.org, invited_by=self.owner, email='b@example.com')
    ...
    def test_accept_creates_membership_and_marks_accepted(self):
        invite = make_invitation(organisation=self.org, invited_by=self.owner, email='new@example.com')
        new_user = make_user(email='new@example.com')
        invite.accept(new_user)
        self.assertEqual(invite.status, Invitation.Status.ACCEPTED)
        self.assertTrue(Membership.objects.filter(user=new_user, organisation=self.org).exists())
        
    def test_accept_uses_invitation_role(self): 
        invite = make_invitation(organisation=self.org, invited_by=self.owner, email='new@example.com', role=Membership.Role.ADMIN)
        new_user = make_user(email='new@example.com')
        invite.accept(new_user)
