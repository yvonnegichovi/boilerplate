...
class UpdateMemberRoleSerializer(serializers.ModelSerializer):
    ...
    def validate_role(self, value):
        ...
        if (
            self.instance
            and self.instance.role == Membership.Role.OWNER
            and value != Membership.Role.OWNER
        ):
            logger.debug(
                f"Attempted to change role of owner membership {self.instance.id} to '{value}'."
            )
        ...
        if value == Membership.Role.OWNER:
            logger.debug(
                f"Attempted to assign owner role to membership {self.instance.id}."
            )
            raise serializers.ValidationError(
                "Cannot assign owner role directly. Transfer Ownership first."
            )
        return value
...
class AcceptInvitationSerializer(serializers.Serializer):
    ...
    def validate_token(self, value):
        ...
        """
        try:
            invitation = Invitation.objects.select_related("organisation").get(
                token=value
            )
            if not invitation._is_valid:
                logger.debug(
                    f"Attempted to accept an invalid or expired invitation with token '{value}'."
                )
                raise serializers.ValidationError("This invitation is no longer valid.")
        except Invitation.DoesNotExist:
            logger.debug(
                f"Attempted to accept a non-existent invitation with token '{value}'."
            )
...
class InviteDetailsSerializer(serializers.ModelSerializer):
    ...
    organisation_slug = serializers.CharField(
        source="organisation.slug", read_only=True
    )
